...
export default function WelcomeScreen({ onGetStarted, onLogin, isMobile }: any) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#080C14", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* Background Glow */}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, textAlign: "center" }}>
        ...
        <div style={{ width: "100%", maxWidth: 440, position: "relative", marginBottom: 32 }}>
          <div key={slide} className="fade-up" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ width: isMobile ? "min(300px, 75vw)" : 300, aspectRatio: "9/14", borderRadius: 32, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", boxShadow: `0 40px 100px rgba(0,0,0,0.6), 0 0 40px ${slides[slide].color}15`, marginBottom: 32, position: "relative", background: "#0D1426" }}>
            ...
            <h2 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: isMobile ? 28 : 34, color: "#fff", marginBottom: 12, lineHeight: 1.1, letterSpacing: "-.5px" }}>{slides[slide].title}</h2>
            <p style={{ color: "#94a3b8", fontSize: 16, lineHeight: 1.5, maxWidth: 320 }}>{slides[slide].desc}</p>
          </div>

          {/* Dots */}
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 24 }}>
            {slides.map((_, i) => (
              <div key={i} onClick={() => setSlide(i)} style={{ width: slide === i ? 24 : 8, height: 8, borderRadius: 4, background: slide === i ? slides[i].color : "rgba(255,255,255,0.1)", cursor: "pointer", transition: "all 0.3s" }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ position: "absolute", top: isMobile ? 40 : 60, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 18, color: "#fff", letterSpacing: -0.5 }}>VaultPay</span>
        </div>
      </div>

      {/* Action Buttons */}
      <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12, width: "100%", maxWidth: 440, margin: "0 auto 40px" }}>
        <button onClick={onGetStarted} style={{ width: "100%", padding: "16px 0", borderRadius: 16, border: "none", background: "linear-gradient(135deg,#00D4AA,#00b896)", color: "#000", fontWeight: 700, fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          Get Started <ChevronRight size={18} />
        </button>
        <button onClick={onLogin} style={{ width: "100%", padding: "16px 0", borderRadius: 16, border: "1px solid #1E2D4A", background: "transparent", color: "#e2e8f0", fontWeight: 600, fontSize: 15, cursor: "pointer" }}>
      ...
      <div style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", display: "flex", alignItems: "center", gap: 6, opacity: 0.5 }}>
        ...
        <span style={{ color: "#64748b", fontSize: 11, fontWeight: 600, letterSpacing: 0.5 }}>TRUSTED BY 10,000+ USERS</span>
      </div>
    </div>
  );
}
